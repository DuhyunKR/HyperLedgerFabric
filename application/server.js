...
app.get('/car/histroy', async (req, res) => {
  ...
  try {
      ...
      if(!identity) {
          ...
          result_object.error=error("An identity for the user appUser does not exist in the wallet")
      ...
      const contract = network.getContract('basic');
      // 트랜젝션 제출
      const result = await contract.evaluateTransaction('GetHistoryCar', cid);
      console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
      ...
      result_object.content=JSON.parse(result);
